...
### OpenAPI スキーマのカスタマイズ（422削除）

FastAPI は Pydantic バリデーション失敗時に自動的に **422 レスポンス** を OpenAPI スキーマに追加します。
しかし、実装上はハンドラで **400 に変換** しているため、スキーマとの不整合が生じます。

**対策:** `main.py` で OpenAPI 生成時に 422 を削除
...
**結果：** Swagger UI には **400 Bad Request のみが表示** される
...
| No  | テスト内容           | 期待結果                                         | 結果 |
| --- | -------------------- | ------------------------------------------------ | ---- |
| 1   | 422 が削除されている | OpenAPI スキーマに 422 レスポンス定義なし        | ✅    |
| 2   | 400 が存在する       | すべてのバリデーションエラーエンドポイントに 400 | ✅    |
