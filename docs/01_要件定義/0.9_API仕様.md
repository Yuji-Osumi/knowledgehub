...
4. Set-Cookie ヘッダーで JWT を返却
   - Cookie 名: access_token
   - HttpOnly: true
   - Secure: true（本番のみ）
   - SameSite: Lax
   - Max-Age: 86400（24時間）
5. クライアントはログイン成功レスポンスを受信
```
...
2. バックエンドが Cookie から JWT 取得
3. JWT 署名検証（SECRET_KEY）
4. 有効期限チェック（exp）
5. ブラックリスト確認（Redis: blacklist:{jti}）
6. user_id を抽出して API 処理実行
```
...
2. バックエンドが JWT から jti 取得
3. Redis にブラックリスト追加（TTL 24h）
...
**処理内容**
1. Cookie から JWT 取得
2. JWT から jti（トークンID）を抽出
3. Redis にブラックリスト追加：`blacklist:{jti}` → "true" (TTL 24h)
4. Cookie 削除（Max-Age=0）
